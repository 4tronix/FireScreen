...
namespace oled4
{
    ...
    export class Oled
    {
        ...
        set_pos(col: number = 0, page: number = 0)
        {
            ...
            let c = col;            // no zoom for now * (_ZOOM + 1)
    ...
    export function newOled(addr: number, lines: number): Oled
    {
        ...
        oled._cBuf2 = pins.createBuffer(2);
        oled._cBuf3 = pins.createBuffer(3);
        oled._cBuf4 = pins.createBuffer(4);
        oled._address = addr;
        oled._is128 = true;
        oled._zoomed = false;
        oled._invert = false;
        oled.cmd1(0xAE);          // DISPLAYOFF
        oled.cmd1(0xA4);          // DISPLAYALLON_RESUME
        oled.cmd2(0xD5, 0xF0);    // SETDISPLAYCLOCKDIV
        oled.cmd2(0xA8, 0x3F);    // SETMULTIPLEX
        oled.cmd2(0xD3, 0x00);    // SETDISPLAYOFFSET
        oled.cmd1(0 | 0x0);       // SETSTARTLINE
        oled.cmd2(0x8D, 0x14);    // CHARGEPUMP
        oled.cmd2(0x20, 0x00);    // MEMORYMODE
        oled.cmd3(0x21, 0, 127);  // COLUMNADDR
        oled.cmd3(0x22, 0, 63);   // PAGEADDR
        oled.cmd1(0xa0 | 0x1);    // SEGREMAP
        oled.cmd1(0xc8);          // COMSCANDEC
        oled.cmd2(0xDA, 0x12);    // SETCOMPINS
        oled.cmd2(0x81, 0xCF);    // SETCONTRAST
        oled.cmd2(0xd9, 0xF1);    // SETPRECHARGE
        oled.cmd2(0xDB, 0x40);    // SETVCOMDETECT
        oled.cmd1(0xA6);          // NORMALDISPLAY
        oled.cmd2(0xD6, 1);       // ZOOM
        oled.cmd1(0xAF) ;         // DISPLAYON
        oled.clearOled();
        return oled;
    }
